...
modis_archive_path <- "C:/Users/AhmadWaseem/Desktop/bhalli/worldpop rf/MODIS_ARC/"
modis_out_path <- "C:/Users/AhmadWaseem/Desktop/bhalli/worldpop rf/MODIS_ARC/PROCESSED/"
...
# install.packages("C:/Users/AhmadWaseem/Desktop/MODIS", repos=NULL, type="source")
...
MODISoptions(gdalPath = 'C:/Users/AhmadWaseem/Desktop/bhalli/worldpop rf/FWTools/bin')
...
if (("NPP" %in% names(metadata)) & ("NPP" %in% default_datasets) & (is.na(dataset_name))) {
	...
	if (proj4string(census) != "+proj=longlat +datum=WGS84 +no_defs") {
		print(proj4string(census))
		stop("ERROR:  The census data shapefile must be projected to GCS WGS84 in order to proceed!")
	}


	##	To just download the tiles:
	# getHdf(product="MOD17A3", begin="2010-01-01", end="2011-01-01", extent=extent(census), localArcPath=modis_out_path)
