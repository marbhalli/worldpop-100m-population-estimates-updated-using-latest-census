...
##		variables:
source("C:/Users/AhmadWaseem/Desktop/bhalli/worldpop rf/src/01.0 - Configuration.py.r")
...
python_path <- "C:\\Python27\\ArcGIS10.8"
...
if (file.exists("census_covariates.shp") & overwrite_compiled_covariates==FALSE) {
	...
	census_data$ADMINID <- as.numeric(unlist(census_data$ADMINID))

	census_data$ADMINPOP <- as.numeric(unlist(census_data$ADMINPOP))

	census_data$YEARPOP <- as.numeric(unlist(census_data$YEARPOP))
	...
	for ( i in 1:length(covariates) ) {
		...
		# if (var_name == "urb_cls" || var_name == "urb_dst"){
		# 	next
		# }
		...
		if (doit) {
			if (dataset_summary == "modal" & gsub("cls", "prp", var_name) %in% names(census_data)) {
				...
				output_stats <- merge(as.data.frame(census_data)['ADMINID'], output_stats, by.x='ADMINID',by.y='zone', sort=FALSE)
...
if (estimate_RF) {
	...
	if (!is.null(fixed_set)) {
		...
		# covariates['urb_cls'] = NULL
		# covariates['urb_dst'] = NULL
	...
	# my line of code, removing columns with ALL NA
	x_data <- x_data[,colSums(is.na(x_data))<nrow(x_data)]
	...
	# x_data <- x_data[,!grepl("40", names(x_data))]
	# x_data <- x_data[,!grepl("130", names(x_data))]
	# x_data <- x_data[,!grepl("150", names(x_data))]
	# x_data <- x_data[,!grepl("160", names(x_data))]
	# x_data <- x_data[,!grepl("230", names(x_data))]
	# x_data <- x_data[,!grepl("250", names(x_data))]
	# x_data <- x_data[,!grepl("011", names(x_data))]
	# x_data <- x_data[,!grepl("200", names(x_data))]
	# x_data <- x_data[,!grepl("210", names(x_data))]
...
#
	...
	init_popfit = tuneRF(x=x_data, y=y_data, plot=TRUE , improve=0.0001, stepFactor=1.20, trace=TRUE, doBest=TRUE, na.action=na.omit, importance=TRUE, proximity=FALSE, replace=TRUE) 
	print(init_popfit)
	...
	if (is.null(fixed_set)) {
		...
		while (length(pos_importance) < length(importance_scores[,1])) {
			...
			pos_importance <- pos_importance[!is.na(pos_importance)]
			...
			popfit = tuneRF(x=x_data, y=y_data, plot=TRUE , improve=0.0001, stepFactor=1.20, trace=TRUE, doBest=TRUE, na.action=na.omit, importance=TRUE, proximity=FALSE, replace=TRUE) 
